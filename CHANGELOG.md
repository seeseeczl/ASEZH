...
- 所有显示使用副本，保留 Shader 标识符、可编辑值、渲染平台名和选项索引。
- 增加跨模板词条审计和控件副本测试，保留各管线实际验证与未验证范围的区别。
- 变更：`shader-settings-localization`；证据见 `docs/04-delivery/shader-settings-0.0.12.md`。

## 0.0.11 — 2026-09-13

- 原生节点标题、输入/输出标签及尺寸测量汉化，补齐模板主节点端口和动态编号标签。
- 精确原生类型白名单与 Flyme 分类排除；自定义函数、编辑值和 Shader 标识符保持原值。
- 语言切换刷新几何缓存；新增显示钩子的安装、升级、重复应用与撤回检查。
- 变更：`native-node-port-localization`。验证范围和目标环境差异见 `docs/04-delivery/native-node-display-0.0.11.md`。
